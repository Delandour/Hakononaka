...
screen say(who, what, narration=False):
    # 正确写法：用括号包裹条件表达式，作为style的属性值
    window:
        id "window"
    ...
    if renpy.variant("small"):
        # 手机设备：缩小头像并调整位置（示例值，根据实际界面微调）
        add SideImage() xalign 0.24 yalign 0.99 zoom 1.0
    else:
        # 非手机设备：使用原来的参数
        add SideImage() xalign 0.24 yalign 1.0
...
style say_narration is say_dialogue:  # 新增旁白专用样式
    xpos gui.dialogue_pb_xpos 
    xsize gui.dialogue_pb_width
...
style quick_button1 is default:
    idle_background Transform(Frame("gui/button/quick_idle.png", 10, 10, tile=True), alpha=0.75)    
    hover_background Transform(Frame("gui/button/quick_hover.png", 10, 10, tile=True), alpha=0.75)  # 通用悬停背景
    selected_background Transform(Frame("gui/button/quick_hover.png", 10, 10, tile=True), alpha=0.75)  # 通用悬停背景
    insensitive_background Frame("quick_insensitive.png", 10, 10, tile=True)  # 通用悬停背景
    xalign 0.4
    yalign 0.5
    xpadding 20
    ypadding 10
    ...
    # 添加文本居中属性
    text_align 0.5      # 文本水平居中


style quick_button1_text:
    #font "title_song3.ttf"  # 字体文件路径（替换为实际路径）
    color "#303139"  # 字体颜色（可选，保持与原有设置一致）
    hover_color "#1e1e42"
    selected_color "#1f1f41"
    xalign 0.5
    yalign 0.5
...
screen quick_menu():
    ...
    if quick_menu:
        ...
        vbox:
            ...
            # 新增的月的房间按钮
            if Y_room == True:
                textbutton _("月的房间") action [Function(randomize_Light_state), Show("Light_room")]:
                    style "quick_button1"

            #textbutton _("回 退") action Rollback():
            #    style "quick_button1" 

            textbutton _("历 史") action ShowMenu('history'):
                style "quick_button1"
           
            textbutton _("快 进") action Skip() alternate Skip(fast=True, confirm=True):
                style "quick_button1"
           
            textbutton _("自 动") action Preference("auto-forward", "toggle"):
                style "quick_button1"
            
            textbutton _("保 存") action ShowMenu('save'):
                style "quick_button1"
            
            #textbutton _("快 存") action QuickSave()
            #textbutton _("快 读") action QuickLoad()
           
            textbutton _("设 置") action ShowMenu('preferences'):
                style "quick_button1"
...
style navigation_button1 is default:
    idle_background Transform(Frame("gui/button/quick_idle.png", 10, 10, tile=True), alpha=0.75)    
    hover_background Transform(Frame("gui/button/quick_hover.png", 10, 10, tile=True), alpha=0.75)  # 通用悬停背景
    selected_background Transform(Frame("gui/button/quick_hover.png", 10, 10, tile=True), alpha=0.75)  # 通用悬停背景
    insensitive_background Frame("quick_insensitive.png", 10, 10, tile=True)  # 通用悬停背景
...
screen navigation():
    ...
    vbox:
        ...
        if renpy.variant("small"):
            xalign 0.02  # 水平居中
            yalign 0.7  # 垂直位置在屏幕60%处
            spacing 30  # 增大按钮间距，适合触摸

        if main_menu:

            textbutton _("开始游戏") action Start():
                style "navigation_button1"
        ...
        if main_menu:
            textbutton _("画廊") action ShowMenu("gallery"):
                style "navigation_button1"
            textbutton _("额外内容") action Start("extra_story"):
                style "navigation_button1"
...
screen main_menu():
    ...
    if persistent.true_end_title:
        add gui.main_menu_background_te
    else:
...
style main_menu_frame:
    xsize 420
    yfill True
    # background "gui/overlay/less black.png"
...
screen preferences():
    ...
    use game_menu(_("设置"), scroll="viewport", yinitial=0.0, spacing=10):
        ...
        vbox:
            ...
            hbox:
                ...
                ## 额外的创建者定义的偏好设置。
            ## ———— 新增：语言切换选项 ————
            null height (2 * gui.pref_spacing)  # 增加小间距，区分上下设置组
            hbox:
                box_wrap True  # 适配不同屏幕宽度，自动换行
                vbox:
                    style_prefix "radio"  # 用“单选”样式（选中后有标记，更直观）
                    label _("语言")  # 语言标题（后续需在翻译文件中翻译）
                    textbutton _("中文") action Language(None)  # 切换回原语言（中文，标识符为None）
                    textbutton _("英语") action Language("english")  # 切换到英语（匹配之前的翻译文件标识符）
                    textbutton _("日语") action Language("japanese")  # 切换到日语（匹配之前的翻译文件标识符）
            ## ———— 新增结束 ————
...
style main_menu_frame:
    variant "small"
    # background "gui/phone/overlay/less black.png"
